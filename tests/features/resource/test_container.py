...
class TestContentRegistry:
    def test_init(self):
        ...
        registry = ContentRegistry(content, "file://test.txt", {"name": "test"})

        assert registry.content == content
        assert registry.uri == "file://test.txt"
        assert registry.extra == {"name": "test"}

    def test_init_without_extra(self):
        content = ResourceContent()
        registry = ContentRegistry(content, "file://test.txt")
...
class TestResourceContainer:
    ...
    def test_add_resource_success(self):
        content = ResourceContent()
        content.add_text_content("Hello", "text/plain")

        registry = self.container.add_resource("file://test.txt", content, name="test")
    ...
    def test_add_resource_invalid_content(self):
        with pytest.raises(
            ValueError, match="Content must be a ResourceContent object"
        ):
    ...
    def test_extract_path_params_content_registry(self):
        content = ResourceContent()
        self.container.add_resource("file://test", content)
        registry = self.container.registrations["file://test"]
    ...
    def test_call_content_registry(self):
        content = ResourceContent()
        content.add_text_content("Hello world", "text/plain")

        self.container.add_resource("file://test.txt", content)
    ...
    def test_call_function_registry(self):
        def sample_func():
            content = ResourceContent()
            content.add_text_content("Generated content", "text/plain")
            return content
    ...
    def test_call_function_with_path_params(self):
        def sample_func(name: str):
            content = ResourceContent()
            content.add_text_content(f"Hello {name}", "text/plain")
            return content
    ...
    def test_build_template_list_result_schema(self):
        def sample_func(param: str):
            content = ResourceContent()
            content.add_text_content(f"Hello {param}", "text/plain")
            return content
    ...
    def test_add_http_resource_without_content(self):
        ...
        assert registry.content is None
        assert (
    ...
    def test_add_http_resource_with_content(self):
        ...
        content = ResourceContent()
        content.add_text_content("API response", "application/json")

        registry = self.container.add_resource(
            "https://example.com/api/data", content, name="API Data"
        )
